// React Native modules are not available under Jest.
...
describe('trackLinkOpen', () => {
  ...
  const analytics = () =>
    new Analytics(
      new HttpClient({ apiKey: 'tolk_pub_test', baseUrl: 'https://links.example.com', debug: false, timeout: 5000 }),
    );

  const posted = () => fetchMock.mock.calls.map(c => String(c[0]));

  beforeEach(() => {
    fetchMock = jest.fn().mockResolvedValue(reply({ attribute: true }));
